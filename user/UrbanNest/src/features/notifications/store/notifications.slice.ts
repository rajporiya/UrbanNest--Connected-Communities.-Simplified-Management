...
const slice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(markNotificationRead.fulfilled, (state, action) => { state.mutationLoading = false; state.items = state.items.filter((item) => item.id !== action.payload.id); state.unreadCount = Math.max(0, state.unreadCount - 1); state.pagination.total = Math.max(0, state.pagination.total - 1) })
      .addCase(markAllNotificationsRead.fulfilled, (state) => { state.mutationLoading = false; state.items = []; state.unreadCount = 0; state.pagination.total = 0 })
