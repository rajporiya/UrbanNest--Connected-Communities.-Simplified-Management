...
export function EventForm({ mode, initialValues, submitting = false, onSubmit, onCancel }: EventFormProps) {
  ...
  return <form noValidate className="space-y-8" onSubmit={form.handleSubmit(onSubmit)}>
    <FormSection title="Event details" description={mode === "create" ? "Add a clear event name and audience." : "Add a clear event name, audience, and publishing status."} icon={<CalendarDays />} columns={2} divider>
      ...
      {mode === "edit" ? <div className="space-y-1.5"><RequiredLabel htmlFor="event-status" required>Status</RequiredLabel><select id="event-status" className={control} {...form.register("status")}><option value="draft">Draft</option><option value="upcoming">Upcoming</option><option value="ongoing">Ongoing</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select></div> : null}
